...
async function main() {
  ...
  if (mintVerifierAddr === null || mintVerifierAddr === undefined)
    throw new Error("Verifier address not found");

  console.log(`✅ Mint Verifier Contract: ${mintVerifierAddr}`);

  const burnVerifierAddr = await deployBin(
    "BurnVerifierV2.bin",
    publicClient,
    walletClient,
  );

  if (burnVerifierAddr === null || burnVerifierAddr === undefined)
    throw new Error("Verifier address not found");

  console.log(`✅ Burn Verifier Contract: ${burnVerifierAddr}`);
}
