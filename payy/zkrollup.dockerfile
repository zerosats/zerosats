...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    if RUSTC_WRAPPER=/usr/local/cargo/bin/sccache cargo build --bin node --bin payy-cli --bin burn-substitutor ${build_flags}; then \
    sccache --show-stats && \
    sccache --stop-server; \
    else \
    status=$?; \
    ...
    cargo build --bin node --bin payy-cli --bin burn-substitutor ${build_flags}; \
    fi; \
    else \
