...
async fn handle_receive(
    ...
    // Use the actual note value instead of hardcoded 1
    let note_value = input_note.note.value.to_u256().as_u64();
    let note: Note = client.get_wallet_mut().receive_note(note_value, chain, token);
