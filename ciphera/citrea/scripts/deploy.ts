...
import {citreaDevChain, citreaTestChain, deployBin} from "./shared";
...
async function main() {
    ...
    // Interrupt execution to verify everything
    await new Promise(r => import('readline').then(rl => rl.createInterface({input: process.stdin}).question('Press Enter...', r)));
