// Register a new noteKind → token address mapping on the rollup. Always
...
async function main() {
  ...
  const newKind = requireEnv("NEW_KIND") as `0x${string}`;
  const newToken = requireEnv("NEW_TOKEN") as `0x${string}`;
  ...
  await assertChainId(publicClient, profile.chainId, "add-token");
