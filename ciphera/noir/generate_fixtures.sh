...
if [ -n "$AGG_AGG_SOL_VK_HASH" ]; then
    ...
    # Update rollup.rs (scope to the verifyRollup function — hex is 3 lines after the marker)
    sed -i.bak "/verifyRollup/{n;n;n;s|\"0x[0-9a-fA-F]\{64\}\"|\"$AGG_AGG_SOL_VK_HASH\"|;}" $REPO_ROOT/pkg/contracts/src/rollup.rs
