use element::Element;
use minimal_poseidon::{Bn256Fr as Base, ConstantLength, Hash, P128Pow5T3};
...
pub fn hash_merge<const N: usize>(elements: [Element; N]) -> Element {
    ...
    let hash = H::<N>::init().hash(elements.map(|e| Base::from_raw(e.to_u64_array())));
    Element::from_le_bytes(hash.to_bytes())
}
