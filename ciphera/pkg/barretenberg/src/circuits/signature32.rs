...
use zk_primitives::{
    Signature32, Signature32Proof, Signature32ProofBytes, Signature32PublicInput, bytes_to_elements,
};
...
mod tests {
    ...
    fn test_signature_proof_generation_and_verification() {
        ...
        let signature = Signature32 { preimage, message };
    ...
    fn test_abc_statechain_transfer() {
        ...
        let adaptor_secret_a: Scalar =
            "11477400e59c41025e4e18c4de244b90b14554dcdcbfa396ead4659aa6343249"
                .parse()
                .unwrap();
        let adaptor_secret_b: Scalar =
            "22477400e59c41025e4e18c4de244b90b14554dcdcbfa396ead4659aa6343249"
                .parse()
                .unwrap();
        let adaptor_secret_c: Scalar =
            "33477400e59c41025e4e18c4de244b90b14554dcdcbfa396ead4659aa6343249"
                .parse()
                .unwrap();
        ...
        let pubkeys_bc = [seckey_x.base_point_mul(), seckey_c.base_point_mul()];
        ...
        let pubkeys_ax = [seckey_a.base_point_mul(), seckey_x.base_point_mul()];
        ...
        musig2::verify_single(aggregated_pubkey_bc, valid_sig_bc, &statechain_message)
        ...
        assert_eq!(
            revealed_abc,
            MaybeScalar::Valid(adaptor_secret_abc.unwrap())
        );

        // C knows #c and now knows #abc, so C can derive #ab = #abc - #c
        // More importantly, C can now adapt the AX signature
        let valid_sig_ax: musig2::LiftedSignature =
            adaptor_sig_ax.adapt(revealed_abc.unwrap()).unwrap();

        musig2::verify_single(aggregated_pubkey_ax, valid_sig_ax, &message)
    ...
    fn test_zcash_note_adaptor() {
        ...
        let valid_sig_bc: musig2::LiftedSignature =
            adaptor_sig_bc.adapt(adaptor_secret_bc).unwrap();
        musig2::verify_single(aggregated_pubkey_bc, valid_sig_bc, &message)
            .expect("invalid decrypted BC signature");
    }
